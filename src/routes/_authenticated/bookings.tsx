...
import {
  cancelAccessoryBooking, listAccessoryBookings, markAccessoryBookingReturned, adminUpdateAccessoryBooking,
} from "@/lib/accessory-bookings";
...
type Kind = "equipment" | "accessory";

/** Unified view-model over equipment and accessory bookings so the page renders one list. */
interface UnifiedBooking {
  ...
  profile: { full_name?: string | null; department?: string | null; employee_id?: string | null } | null;
}

function BookingsPage() {
  const { data: user, isLoading: userLoading } = useSessionUser();
  ...
  // Non-privileged users are always scoped to their own bookings (defaulting via useState
  // from `canManage` would be stale on first render before the session resolves).
  const effectiveScope: "mine" | "all" = canManage ? scope : "mine";
  const managerDepartment = isManager ? user?.department : undefined;
  const statusArg = status === "all" ? undefined : status;

  const eqQuery = useQuery({
    queryKey: ["bookings", effectiveScope, status, user?.id, user?.department],
    queryFn: () => listBookings({ scope: effectiveScope, status: statusArg, limit: 200, managerDepartment }),
    enabled: !!user,
  });
  const accQuery = useQuery({
    queryKey: ["accessory-bookings", effectiveScope, status, user?.id, user?.department],
    queryFn: () => listAccessoryBookings({ scope: effectiveScope, status: statusArg, limit: 200, managerDepartment }),
    enabled: !!user,
  });

  const loading = userLoading || eqQuery.isLoading || accQuery.isLoading;
  const errored = eqQuery.isError || accQuery.isError;

  const unified: UnifiedBooking[] = [
    ...(eqQuery.data?.rows ?? []).map((b): UnifiedBooking => ({
      kind: "equipment", id: b.id, user_id: b.user_id, status: b.status,
      booking_date: b.booking_date, end_date: b.end_date, start_time: b.start_time, end_time: b.end_time,
      quantity: b.quantity, project_name: b.project_name, purpose: b.purpose,
      itemName: b.equipment?.name ?? "—", itemCode: b.equipment?.equipment_code ?? null, profile: b.profile,
    })),
    ...(accQuery.data?.rows ?? []).map((b): UnifiedBooking => ({
      kind: "accessory", id: b.id, user_id: b.user_id, status: b.status,
      booking_date: b.booking_date, end_date: b.end_date, start_time: b.start_time, end_time: b.end_time,
      quantity: b.quantity, project_name: b.project_name, purpose: b.purpose,
      itemName: b.accessory?.description ?? "—", itemCode: null, profile: b.profile,
    })),
  ...
  const cancel = useMutation({
    mutationFn: ({ b, reason }: { b: UnifiedBooking; reason?: string }) =>
      b.kind === "equipment" ? cancelBooking(b.id, reason) : cancelAccessoryBooking(b.id, reason),
    onSuccess: () => { toast.success("Booking cancelled"); invalidateBookingRelatedQueries(qc); },
    onError: (e: Error) => toast.error(e.message),
  });
  const ret = useMutation({
    mutationFn: ({ b, reason }: { b: UnifiedBooking; reason?: string }) =>
      b.kind === "equipment" ? markReturned(b.id, reason) : markAccessoryBookingReturned(b.id, reason),
    onSuccess: () => { toast.success("Marked returned"); invalidateBookingRelatedQueries(qc); },
    onError: (e: Error) => toast.error(e.message),
  });

  function canCancel(b: UnifiedBooking) {
  ...
  function canReturn(b: UnifiedBooking) {
    if (b.status !== "booked") return false;
    if (canManage) return true;
    if (b.user_id !== user?.id) return false;
    return new Date(`${b.booking_date}T${b.start_time}`) <= new Date();
  ...
  function exportBookings() {
    const ws = XLSX.utils.json_to_sheet(filtered.map((b) => ({
      "Booking ID": b.id,
      Type: b.kind === "equipment" ? "Equipment" : "Accessory",
      Item: b.itemName,
      "Asset ID": b.itemCode ?? "",
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-xl sm:text-2xl font-semibold">{canManage ? "Bookings" : "My Bookings"}</h1>
          <p className="text-sm text-muted-foreground">Equipment and accessory reservations.</p>
        </div>
      ...
      <Card>
        <CardContent className="p-4 flex flex-wrap gap-3 items-center">
          ...
          <Input placeholder="Search item, user, project, booking id…" value={search} onChange={(e) => setSearch(e.target.value)} className="w-full sm:w-64" />
      ...
      {loading ? (
        <Card><div className="flex items-center justify-center gap-2 py-12 text-muted-foreground text-sm"><Loader2 className="h-4 w-4 animate-spin" /> Loading bookings…</div></Card>
      ) : errored ? (
        <Card><div className="text-center py-10 text-destructive text-sm">Couldn't load bookings. Please try again.</div></Card>
      ) : filtered.length === 0 ? (
        <Card><div className="text-center py-12 text-muted-foreground text-sm">No active bookings.</div></Card>
      ) : (
        <>
          {/* Mobile: card list */}
                ...
                <CardContent className="p-4 space-y-2">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground">{b.itemCode ?? (b.kind === "accessory" ? "Accessory" : "")}</div>
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      <TypeBadge kind={b.kind} />
                  ...
                  {(canCancel(b) || canReturn(b)) && b.status === "booked" && (
                    <div className="flex items-center gap-2 pt-1 flex-wrap">
                      {canReturn(b) && <ReturnButton onConfirm={(reason) => ret.mutate({ b, reason })} pending={ret.isPending} className="flex-1" label={canManage && b.user_id !== user?.id ? "Force Return" : "Return"} />}
                      {canCancel(b) && <CancelButton onConfirm={(reason) => cancel.mutate({ b, reason })} pending={cancel.isPending} className="flex-1" />}
            ...
            <CardContent className="p-0 overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y">
                  {filtered.map((b) => (
                    <tr key={`${b.kind}-${b.id}`} className="hover:bg-muted/30">
                      <td className="px-4 py-3"><TypeBadge kind={b.kind} /></td>
                      <td className="px-4 py-3">
                        <div className="font-medium">{b.itemName}</div>
                        {b.itemCode && <div className="text-xs text-muted-foreground">{b.itemCode}</div>}
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-right space-x-2 whitespace-nowrap">
                        {b.status === "booked" && canReturn(b) && (
                          <ReturnButton onConfirm={(reason) => ret.mutate({ b, reason })} pending={ret.isPending} label={canManage && b.user_id !== user?.id ? "Force Return" : "Return"} />
                        )}
                        {b.status === "booked" && canCancel(b) && (
                          <CancelButton onConfirm={(reason) => cancel.mutate({ b, reason })} pending={cancel.isPending} />
                        )}
...
function TypeBadge({ kind }: { kind: Kind }) {
  return <span className="inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-medium capitalize text-muted-foreground">{kind}</span>;
}
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            This ends the booking before its scheduled end and frees the item immediately. This cannot be undone.
          </AlertDialogDescription>
        </AlertDialogHeader>
...
function EditBookingDialog({ booking, onDone }: { booking: UnifiedBooking; onDone: () => void }) {
  ...
  const mut = useMutation({
    mutationFn: async () => {
      const args = {
        bookingId: booking.id, quantity, booking_date: bookingDate, end_date: endDate,
        start_time: startTime, end_time: endTime, project_name: projectName, purpose, override,
      };
      if (booking.kind === "equipment") await adminUpdateBooking(args);
      else await adminUpdateAccessoryBooking(args);
    },
    onSuccess: () => { toast.success("Booking updated"); setOpen(false); onDone(); },
    onError: (e: Error) => {
      const map: Record<string, string> = {
        insufficient_quantity: "Not enough available for that slot — enable Override to force it through",
        ...
        equipment_unavailable: "Item is not active — enable Override to force it through",
        accessory_unavailable: "Item is not active — enable Override to force it through",
      };
